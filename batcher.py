...
class Example(object):
    ...
    def __init__(self, title, summarization, utterance, vocab, hps):
        ...
        utterance_words = split_text_with_whitespace(utterance) + [MARK_EOS] # + ' '
        self.utterance_len = len(utterance_words)
        self.utterance_input = [vocab.word2id(w) for w in utterance_words]
        ...
        self.enc_input_extend_vocab, self.oovs = title2ids(title_words, vocab)
        self.utterance_input_extend_vocab, self.oovs = utterance2ids(utterance_words, vocab, self.oovs)
	...
	self.original_utterance = utterance
    ...
    def pad_utterance_input(self, max_len, pad_id):
        self.utterance_input += [pad_id] * (max_len - len(self.utterance_input))
        self.utterance_input_extend_vocab += [pad_id] * (
            max_len - len(self.utterance_input_extend_vocab))
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list, hps):
        ...
        max_utterance_seq_len = max([ex.utterance_len for ex in example_list])

        # Pad the encoder input sequences up to the length of the longest sequence
        for ex in example_list:
            ex.pad_encoder_input(max_enc_seq_len, self.pad_id)
            ex.pad_utterance_input(max_utterance_seq_len, self.pad_id)
        ...
        # utterance encoder part
        self.utterance_batch = np.zeros((hps.batch_size, max_utterance_seq_len), dtype=np.int32)
        self.utterance_lens = np.zeros((hps.batch_size), dtype=np.int32)
        self.utterance_padding_mask = np.zeros((hps.batch_size, max_utterance_seq_len), dtype=np.float32)
        ...
        for i, ex in enumerate(example_list):
            ...
            self.utterance_batch[i, :] = ex.utterance_input[:]
            self.utterance_lens[i] = ex.utterance_len
            for j in xrange(ex.utterance_len):
                self.utterance_padding_mask[i][j] = 1
        ...
        if hps.pointer_gen:
            ...
            self.utterance_batch_extend_vocab = np.zeros(
                (hps.batch_size, max_utterance_seq_len), dtype=np.int32)
            for i, ex in enumerate(example_list):
                self.enc_batch_extend_vocab[i, :] = ex.enc_input_extend_vocab[:]
                self.utterance_batch_extend_vocab[i, :] = ex.utterance_input_extend_vocab[:]
    ...
    def store_orig_strings(self, example_list):
        ...
        self.original_utterances = [ex.original_utterance for ex in example_list]
...
class Batcher(object):
    ...
    def fill_example_queue(self):
        ...
        while True:
            try:
                (title, summarization, utterance) = input_gen.next()
            ...
            example = Example(title, summarization, utterance, self._vocab, self._hps)
